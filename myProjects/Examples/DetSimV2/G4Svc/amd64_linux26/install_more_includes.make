...
#
#   Generated Fri Nov 20 00:09:11 2020  by lint
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(G4Svc_tag)_install_more_includes.make
cmt_local_tagfile_install_more_includes = $(bin)$(G4Svc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(G4Svc_tag).make
cmt_local_tagfile_install_more_includes = $(bin)$(G4Svc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)G4Svc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)G4Svcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "G4Svc" = ""; then \
	    if test -d ../G4Svc; then \
	      echo "Installing files from more ../G4Svc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/G4Svc; then mkdir -p $(install_include_dir)/G4Svc; fi;\
	        $(install_command) ../G4Svc $(install_include_dir);\
	    else \
	      echo "No more G4Svc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/G4Svc"; \
	  $(uninstall_command) "$(install_include_dir)/G4Svc/*" ; \
	  if test -f $(install_include_dir)/G4Svc.cmtref ; then \
	    echo "Removing $(install_include_dir)/G4Svc.cmtref"; \
	    eval rm -f $(install_include_dir)/G4Svc.cmtref ; \
	  fi; \
	fi
