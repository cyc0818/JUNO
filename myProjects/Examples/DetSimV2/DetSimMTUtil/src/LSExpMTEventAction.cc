...
void LSExpMTEventAction::BeginOfEventAction(const G4Event* evt)
{
    ...
    if (evt->GetUserInformation()) {
        ...
        if (eui) {
            GlobalBuffer::Elem* elem = eui->elem;
            JM::EvtNavigator* nav = nullptr;
            JM::SimHeader* simhdr = nullptr;
            JM::SimEvent* simevt = nullptr;
            
            if (elem) {
                nav = static_cast<JM::EvtNavigator*>(elem->dptr);
            }
            if (nav) {
                simhdr = dynamic_cast<JM::SimHeader*>(nav->getHeader("/Event/Sim"));
            }
            if (simhdr) {
                simevt = dynamic_cast<JM::SimEvent*>(simhdr->event());
            }
            if (simevt) {
