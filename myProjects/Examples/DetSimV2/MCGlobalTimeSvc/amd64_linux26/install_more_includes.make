...
#
#   Generated Fri Nov 20 00:09:26 2020  by lint
#
#====================================
...
#cmt_local_tagfile_install_more_includes = $(MCGlobalTimeSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(MCGlobalTimeSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)MCGlobalTimeSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)MCGlobalTimeSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "MCGlobalTimeSvc" = ""; then \
	    if test -d ../MCGlobalTimeSvc; then \
	      echo "Installing files from more ../MCGlobalTimeSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/MCGlobalTimeSvc; then mkdir -p $(install_include_dir)/MCGlobalTimeSvc; fi;\
	        $(install_command) ../MCGlobalTimeSvc $(install_include_dir);\
	    else \
	      echo "No more MCGlobalTimeSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/MCGlobalTimeSvc"; \
	  $(uninstall_command) "$(install_include_dir)/MCGlobalTimeSvc/*" ; \
	  if test -f $(install_include_dir)/MCGlobalTimeSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/MCGlobalTimeSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/MCGlobalTimeSvc.cmtref ; \
	  fi; \
	fi
