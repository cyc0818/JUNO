#include "CalibTubeConstruction.hh"
...
DECLARE_TOOL(CalibTubeConstruction);

CalibTubeConstruction::CalibTubeConstruction(const std::string& name)
    : ToolBase(name)
      , solidCT(0), logicCT(0)
...
CalibTubeConstruction::~CalibTubeConstruction() {

}

G4LogicalVolume*
CalibTubeConstruction::getLV() {
    if (logicCT) {
        return logicCT;
    }
...
bool
CalibTubeConstruction::inject(std::string motherName, IDetElement* other, IDetElementPos* pos) {
    return true;
}

void
CalibTubeConstruction::initVariables() {
...
void 
CalibTubeConstruction::initMaterials() {
    // Xin Qian added Nov 2nd 2014 
...
void
CalibTubeConstruction::makeCTLogical() {
  // main CT tube volume
