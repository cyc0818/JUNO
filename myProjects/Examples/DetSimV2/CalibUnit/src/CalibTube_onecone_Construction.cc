#include "CalibTube_onecone_Construction.hh"
...
DECLARE_TOOL(CalibTube_onecone_Construction);

CalibTube_onecone_Construction::CalibTube_onecone_Construction(const std::string& name)
    : ToolBase(name)
      , solidCT(0), logicCT(0)
...
CalibTube_onecone_Construction::~CalibTube_onecone_Construction() {

}

G4LogicalVolume*
CalibTube_onecone_Construction::getLV() {
    if (logicCT) {
        return logicCT;
    }
...
bool
CalibTube_onecone_Construction::inject(std::string motherName, IDetElement* other, IDetElementPos* pos) {
    return true;
}

void
CalibTube_onecone_Construction::initVariables() {
...
void 
CalibTube_onecone_Construction::initMaterials() {
    // Xin Qian added Nov 2nd 2014 
...
void
CalibTube_onecone_Construction::makeCTLogical() {
  // main CT tube volume
  ...
  solidTube2 = new G4Cons("sTube2",
			  ...
			  0*cm,
			  m_calibTubeOuterRadius,
			  (m_calibTubeLength2)/2.-m_calibTubeThickness,
			  0.*deg,
			  360*deg
			  );
  ...
  physiTube2 = new G4PVPlacement(0,              // no rotation
				 G4ThreeVector(0,0,-m_calibTubeLength1/2.+m_calibTubeThickness), // at (x,y,z)
  ...
  solidTube3 = new G4Tubs("vTube3",
			  m_calibTubeOuterRadius-m_calibTubeThickness,
			  10*cm,
			  m_calibTubeThickness,
			  0.*deg,
			  360*deg
			  );
  logicvacTube3 = new G4LogicalVolume(solidTube3,
				      Acrylic,
				      "lvTube3",
				      0,
				      0,
  ...
  physivacTube3 = new G4PVPlacement(0,              // no rotation
				    G4ThreeVector(0,0,-(m_calibTubeLength1+m_calibTubeLength2)/2.+m_calibTubeThickness), // at (x,y,z)
  ...
  physiWindow = new G4PVPlacement(0,              // no rotation
				  G4ThreeVector(0,0,-(m_calibTubeLength2)/2.+m_calibTubeThickness), // at (x,y,z)
