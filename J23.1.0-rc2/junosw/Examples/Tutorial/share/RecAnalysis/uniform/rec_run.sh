...
function job-top-dir() {
    echo ${TOPDIR:-/junofs/users/liuq/J16v1r2-validation/uniform}
}
...
input=$(gen-name cal $seed $evtmax root)
output=$(gen-name rec $seed $evtmax root)
log=$(gen-name reclog $seed $evtmax txt)

cd $jobdir
source $(setup-script)
(time python $(run-script)/tut_calib2rec.py --evtmax $evtmax \
                           --gdml \
                           --input $input \
                           --output $output \
