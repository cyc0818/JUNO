...
#ifdef PMTSIM_STANDALONE
    top_layer->parameter.sin_theta = std::sin(theta*M_PI/180.);
    top_layer->parameter.cos_theta = std::cos(theta*M_PI/180.);
#else
...
#ifdef PMTSIM_STANDALONE
        layers[i]->parameter.cos_theta = std::sqrt(one - layers[i]->parameter.sin_theta*layers[i]->parameter.sin_theta);
#else
...
#ifdef PMTSIM_STANDALONE
            TComplex delta = 2. * M_PI * ni * di * layers[i]->parameter.cos_theta / wl;
            
            Ms_00 = tmps * std::exp(-zi*delta);
            Ms_01 = tmps * layers[i-1]->parameter.rs_ij * std::exp(zi*delta);
            Ms_10 = tmps * layers[i-1]->parameter.rs_ij * std::exp(-zi*delta);
            Ms_11 = tmps * std::exp(zi*delta);

            Mp_00 = tmpp * std::exp(-zi*delta);
            Mp_01 = tmpp * layers[i-1]->parameter.rp_ij * std::exp(zi*delta);
            Mp_10 = tmpp * layers[i-1]->parameter.rp_ij * std::exp(-zi*delta);
            Mp_11 = tmpp * std::exp(zi*delta);
#else
