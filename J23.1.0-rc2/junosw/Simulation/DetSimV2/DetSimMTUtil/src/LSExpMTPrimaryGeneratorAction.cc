...
LSExpMTPrimaryGeneratorAction::LSExpMTPrimaryGeneratorAction()
{
...
void LSExpMTPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    if (m_isMT && !m_scope) {
        ...
        auto m_gbuf =  JunoGlobalStream::GetBufferFrom("GEvtStream");
        JunoGlobalBuffer::Elem* elem_in_gbuf = m_gbuf->next();

        G4cout << "LSExpMTPrimaryGeneratorAction::GeneratePrimaries: "
               << " Get GlobalBuffer::Elem " << elem_in_gbuf
               << " dptr: " << elem_in_gbuf->dptr
               << G4endl;

        JUNOEventUserInfo* eui = new JUNOEventUserInfo();
        eui->elem = elem_in_gbuf;

        anEvent->SetUserInformation(eui);
        return;
    }
...
HepMC::GenEvent*
LSExpMTPrimaryGeneratorAction::load_gen_event() {
    ...
    JM::GenHeader* gen_header = JM::getHeaderObject<JM::GenHeader>(evt_nav);
    if (not gen_header) {
        return 0;
    }
    JM::GenEvt* gen_event = dynamic_cast<JM::GenEvt*>(gen_header->event());
    if (not gen_event) {
        return 0;
    }
...
void LSExpMTPrimaryGeneratorAction::setMTmode(bool flag) {
    m_isMT = flag;
}
