#include "CalibTube_convexwindow_Construction.hh"
...
DECLARE_TOOL(CalibTube_convexwindow_Construction);

CalibTube_convexwindow_Construction::CalibTube_convexwindow_Construction(const std::string& name)
    : ToolBase(name)
      , solidCT(0), logicCT(0)
...
CalibTube_convexwindow_Construction::~CalibTube_convexwindow_Construction() {

}

G4LogicalVolume*
CalibTube_convexwindow_Construction::getLV() {
    if (logicCT) {
        return logicCT;
    }
...
bool
CalibTube_convexwindow_Construction::inject(std::string motherName, IDetElement* other, IDetElementPos* pos) {
    return true;
}

void
CalibTube_convexwindow_Construction::initVariables() {
...
void 
CalibTube_convexwindow_Construction::initMaterials() {
    // Xin Qian added Nov 2nd 2014 
...
void
CalibTube_convexwindow_Construction::makeCTLogical() {
  // main CT tube volume
  solidCT = new G4Tubs("sCT",
               0*m,
               m_calibTubeOuterRadius,
               (m_calibTubeLength1+m_calibTubeLength2)/2.+m_calibWindowDistortionHeight,
               0.*deg,
               360*deg
               );
  ...
  physiWindow = new G4PVPlacement(0,              // no rotation
                  G4ThreeVector(0,0,-(m_calibTubeLength1+m_calibTubeLength2)/2.), // at (x,y,z)
