...
ellipsoid_shift:5mm
    This is needed to fix bug #33 : by making the ellipsoid curve of the MaskTail
    concentric with the ellipsoid curve of the PMT. 
    The 5 mm shift corresponds to the m2_h shift used within Hamamatsu_R12860_PMTSolid::GetSolid
    to shift the lower hemi-ellipsoid "solid_III" is CSG combination to form "pmt_solid"

r2_widen:2mm 
    most of the bug #33 overlap is fixed by the ellipsoid_shift, however a 
    fraction of a mm of the MaskTail still overlaps the polycone neck of the PMT.   
    Widening the mask neck with r2_widen avoids this small overlap. 
...
HamamatsuMaskManager::makeMaskTailLogical() {
    ...
    G4double ellipsoid_shift = 5.*mm ;  
    G4double r2_widen = 2.*mm ; 

#ifdef PMTSIM_STANDALONE
    if(hasOpt("BUG33")) // see issue #33  
    {
        ellipsoid_shift = 0.*mm ; 
        r2_widen = 0.*mm ; 
    }
#endif
    ...
    Tail_outer_I_Ellipsoid = new G4Ellipsoid(
            ...
            mask_radiu_out, // pySemiAxis
            htop_out,  // pzSemiAxis
            -(height_out+paramRealMaskTail.height) + ellipsoid_shift, // pzBottomCut (TODO)
            -height_out + ellipsoid_shift // pzTopCut
            );
#ifdef PMTSIM_STANDALONE
    ...
    addValue("SolidMaskTail.TailOuterIEllipsoid.pzTopCut.ellipsoid_shift", ellipsoid_shift ) ; 
#endif

    Tail_outer_I_Tube = SimUtil::MakeTubsWithPolycone(
            ...
            0*deg, 
            360*deg, 
            -(height_out+paramRealMaskTail.edge_height/2) );
    ...
    Tail_outer_I = new G4UnionSolid
        (objName()+"Tail_outer_PartI",
         ...
         0,
         G4ThreeVector(0,0,-ellipsoid_shift)) ;

    Tail_outer_II_Tube = new G4Tubs
        (objName()+"Tail_outer_PartII",
         0*mm,
         paramRealMaskTail.r2 + r2_widen,
    ...
    Tail_inner_I_Ellipsoid = new G4Ellipsoid(
            ...
            mask_radiu_in, // pySemiAxis
            htop_in,  // pzSemiAxis
            -(height_out+paramRealMaskTail.height-htop_thickness) + ellipsoid_shift, // pzBottomCut (TODO)
            -height_out + ellipsoid_shift // pzTopCut
            );
#ifdef PMTSIM_STANDALONE
    ...
    Tail_inner_I_Tube = SimUtil::MakeTubsWithPolycone(
    ...
    Tail_inner_II_Tube = new G4Tubs
        (objName()+"Tail_inner_PartII",
         0*mm,
         paramRealMaskTail.r2 - requator_thickness + r2_widen,
