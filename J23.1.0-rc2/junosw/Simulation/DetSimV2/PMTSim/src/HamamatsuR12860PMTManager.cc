 ...
 HamamatsuR12860PMTManager::init_mirror_surface() 
{
    if( m_mirror_opsurf != nullptr ) return ;
        ...
        G4String name ; 
        name += CommonPMTManager::GetMirrorOpticalSurfacePrefix(m_UsePMTNaturalGeometry, m_UsePMTOpticalModel ) ;  // control customization, see above 
        name += GetName() ; 
        name += "_Mirror_opsurf" ; 
...
* geometry same for both m_UsePMTOpticalModel


m_UsePMTNaturalGeometry:false
~~~~~~~~~~~~~~~~~~~~~~~~~~~

There is geometry dependency on m_UsePMTOpticalModel 
...
Creates solids, logical volumes and physical volumes placing them within *parent_log*
which is *inner_log* for m_UsePMTNaturalGeometry:true or inner2_log otherwise.  
