...

{
  ...
  if (aMaterialPropertiesTable->ConstPropertyExists("CerenkovYieldFactor")) {
      MeanNumberOfPhotons *= aMaterialPropertiesTable->GetConstProperty("CerenkovYieldFactor");
  }
...
#endif
#ifdef WITH_G4CXOPTICKS_DEBUG
  dbg.BetaInverse = BetaInverse ; 
  dbg.step_length = step_length ; 
  dbg.MeanNumberOfPhotons = MeanNumberOfPhotons ;  
  dbg.fNumPhotons = fNumPhotons ;  
  dbg.add(); 
#endif
...
#ifdef WITH_G4CXOPTICKS
      U4::GenPhotonEnd(i, aSecondaryTrack);  // sets spho label into Geant4 user track info 
#endif
