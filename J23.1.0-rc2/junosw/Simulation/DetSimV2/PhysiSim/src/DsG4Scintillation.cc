...
#ifdef WITH_G4CXOPTICKS
const plog::Severity DsG4Scintillation::LEVEL = SLOG::EnvLevel("DsG4Scintillation", "DEBUG") ; 
#endif
...
// evenly along the track segment and uniformly into 4pi.
{
  ...
  //-----------------------------------------------------//
    ...
    if (flagReemission) {   
        ...
        // G4double birk2 = (0.0031*g/MeV/cm2)*(0.0031*g/MeV/cm2);

        if(aParticleName == "gamma" || aParticleName == "e+" || aParticleName == "e-"){
            birk1 = birksConstant1[11];
            birk2 = birksConstant2[11];
        }else if(abs(aParticle->GetCharge())<1.5){
            birk1 = birksConstant1[2212];
            birk2 = birksConstant2[2212];
        }else{
            birk1 = birksConstant1[1000020040];
            birk2 = birksConstant2[1000020040];
        }
       ...
       if(FastMu300nsTrick)  {
        ...
        dbg.ResolutionScale = ResolutionScale ; 
        dbg.dx = dx ;
        dbg.dE_dx = dE_dx ; 
        dbg.birk1 = birk1 ; 

        dbg.TotalEnergyDeposit = TotalEnergyDeposit ;
        dbg.QuenchedTotalEnergyDeposit = QuenchedTotalEnergyDeposit ;
        dbg.delta = delta ; 
        dbg.birk2 = birk2 ;  

        dbg.ScintillationYield = ScintillationYield ;
        dbg.MeanNumberOfTracks = MeanNumberOfTracks ; 
        dbg.NumTracks = double(NumTracks) ; 
        dbg.MeanNumberOfPhotons = MeanNumberOfPhotons ;  

        dbg.Density = aMaterial->GetDensity(); 
        dbg.fPhotonWeight = fPhotonWeight ;  
        dbg.fApplyPreQE = fApplyPreQE ;  
        dbg.fEnableQuenching = fEnableQuenching  ;  

        dbg.gammaloss  = aTrack.GetDefinition() == G4Gamma::Gamma() && dE > 0 ; 
        dbg.fPreQE = fPreQE ; 
        dbg.b = 0. ; 
        dbg.c = 0. ; 
        ...
        LOG(LEVEL) << " MeanNumberOfTracks " << MeanNumberOfTracks << " NumTracks " << NumTracks ; 
#endif
...
//-------------------------------------------------//
    ...
    for(size_t scnt = 0 ; scnt < nscnt ; scnt++){
       ...
       //  G4int m_Num =G4int(NumTracks * (*Ratio_timeconstant)[scnt]);
        ...
        if(is_opticks_genstep && (m_opticksMode > 0))
        {    
