...

{
   ...
   // G4String strYieldRatio = "YIELDRATIO";
    ...
    if ( verboseLevel > 0) {
      G4cout << " Fast_Intensity " << Fast_Intensity << " Slow_Intensity " << Slow_Intensity <<G4endl;
    }
 ...
 */
   ...
   // loop for determining the optical photons number of any component.
   ...
   /*
    ...
    for (G4int scnt = 0; scnt < nscnt; scnt++) {
        ...
        //std::cout << " scnt = " <<scnt << " time_constant = " << ScintillationTime << " yield_ratio = "<< (*Ratio_timeconstant)[scnt] <<" m_Num = "<<m_Num[scnt]<<std::endl;

        // Max Scintillation Integral
                
         for(G4int i = 0 ; i < NumVec[scnt] ; i++) { //Num is # of 2ndary tracks now
