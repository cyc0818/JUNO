...
void LSExpPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    for (/*nop*/; doneVtx != iVtx; ++iVtx) {
        ...
        for (/*nop*/; donePart != iPart; ++iPart) {
            ...
            if (istatus == 0x1000 || istatus == 0x2000) {
            ...
            } else if(istatus == 0x4000) { // 0x4000 are the partially contained muon/anti-muon in atmo nu events refer to Simulation/GenTools/src/GtFCSelTool.cc line 64
                // skip the PC event
                return;
            } else if (istatus != 1) {
                continue;
            ...
            if (particle_def == 0 and pdgcode != 20022) {
                ...
                if (!particle_def) {
                    ...
                    } else if (Z == 56 && A == 137 && lvl == 1) { // Cs137->Ba137m
                        E = 661.659*CLHEP::keV;
                ...
                if (particle_def) {
                    if (istatus == 0x1000 || istatus == 0x2000 ) {
                        // it is G4 radioactivity decay
                        // Using G4 to do decay.

                        // Force the decay time to zero for the parent nuclide if istatus == 0x1000
                        if (istatus == 0x1000) { 
                            particle_def->SetPDGLifeTime(0.0);
                        }
                        else{
                            G4cout<< " istatus = "<<istatus
                                  << " don't set decay time to zero ";
                            assert( istatus == 0x2000 );
                        }
                    } else {
                        // it could be GenDecay
...
LSExpPrimaryGeneratorAction::load_gen_event() {
    ...
    auto gen_header = JM::getHeaderObject<JM::GenHeader>(evt_nav);
    if (not gen_header) {
        return 0;
    }
    auto gen_event = dynamic_cast<JM::GenEvt*>(gen_header->event());
    if (not gen_event) {
        return 0;
    }
