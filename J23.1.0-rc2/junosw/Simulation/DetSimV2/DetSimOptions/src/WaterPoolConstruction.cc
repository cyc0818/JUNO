    ...
    : ToolBase(name)
{
    ...
    declProp("enabledLatticedShell", m_enabledLatticedShell=true);
    declProp("IsVetoPMT20inchEnabled", m_pmt20inch_enabled=true);
}
...
WaterPoolConstruction::inject(std::string motherName, IDetElement* other, IDetElementPos* pos) {
    ...
    // if the placed daughters are 20inch PMT, but it is not enabled in wp,
    // just return.
    ...
    if (pos == 0) {
        ...
        while (pos->hasNext()) {
            if (is_20inch_pmt and pmt_param_svc) {
                // veto PMTs:
                ...
                if (pmt_param_svc->isConstructed(copyno)) {
                    new G4PVPlacement(
                                      pos->next(),
                                      daughtervol,
                                      daughtervol->GetName()+"_phys", // todo: need a better name.
                                      mothervol,
                                      false,
                                      copyno++,
    ...
    if(motherName == "OuterWaterVeto") {
        LogInfo << "Veto PMT Number = " << (copyno-kOFFSET_WP_PMT) << std::endl;
    }
