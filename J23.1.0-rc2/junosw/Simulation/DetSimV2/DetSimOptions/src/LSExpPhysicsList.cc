...
LSExpPhysicsList::LSExpPhysicsList() : G4VModularPhysicsList()
{
  ...
  cutForProton    = 0.0*mm; // according to QGSP_BERT_HP
...
void LSExpPhysicsList::ConstructParticle()
{
    ...
    ToolBase* t = 0;
    std::string toolname;
    toolname = "DsG4EmLivermorePhysics";
    t = detsimalg->findTool(toolname);
    if (not t) {
        t = detsimalg->createTool(toolname);
    }
    assert(t);
    emPhysicsList = dynamic_cast<G4VPhysicsConstructor*>(t);
    assert(emPhysicsList);

    emPhysicsList->ConstructParticle();

    toolname = "DsWrapPhysConsIon";
    t = detsimalg->findTool(toolname);
    if (not t) {
        t = detsimalg->createTool(toolname);
    }
    assert(t);
    ionList = dynamic_cast<G4VPhysicsConstructor*>(t);
    assert(ionList);

    ionList->ConstructParticle();

    G4PhotonSource::Definition();
