...
void TimeRelation::Loop()
{
//   In a ROOT session, you can do:
//      Root > .L TimeRelation.C
//      Root > TimeRelation t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   const int Nr = 200; // radial direction
   ...
   TH1F* ths_hittime[Nr][Nt];
   for (int i = 0; i < Nr; ++i) {
       for (int j = 0; j < Nt; ++j) {
           TString th_name = TString::Format("%d", i*Nt+j);
           ths_hittime[i][j] = new TH1F(th_name, th_name, 3000, 0, 3000);
       }
   }
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (1<=binx and binx<=Nr and 1<=biny and biny<=Nt) {
          ths_hittime[binx-1][biny-1]->Fill(hittime);
      }
   }
