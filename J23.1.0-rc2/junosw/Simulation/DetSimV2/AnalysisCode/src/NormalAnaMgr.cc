...
NormalAnaMgr::BeginOfRunAction(const G4Run* /*aRun*/) {
    ...
    m_evt_tree = svc->bookTree(*m_par, "SIMEVT/evt", "evt");
    ...
    m_evt_tree->Branch("LpmtPE", &m_cd_lpmt_totalPE, "LpmtPE/I");
    m_evt_tree->Branch("SpmtPE", &m_cd_spmt_totalPE, "SpmtPE/I");
    m_evt_tree->Branch("NNVTPE", &m_cd_nnvt_totalPE, "NNVTPE/I");
    m_evt_tree->Branch("HamaPE", &m_cd_hamamatsu_totalPE, "HamaPE/I");
    m_evt_tree->Branch("CDPE",&m_cd_totalPE,"CDPE/I");
    m_evt_tree->Branch("WPPE",&m_wp_totalPE,"WPPE/I");
...
NormalAnaMgr::PostUserTrackingAction(const G4Track* aTrack) {
    ...
    // if there is no secondaries, do nothing.
    if(not secondaries) {
        return;
    }
