    ...
    : ToolBase(name)
{
    ...
    declProp("SaveSimTrack", m_savesimtrack = true);
}
  ...
  //  if(col){col_muon=0;}
    ...
    if(col_muon->entries()){
       ...
       if(PMT::IsCD(copyno) and PMT::Is20inch(copyno)) {
           ...
           if( m_pmt_param_svc -> isHamamatsu(copyno)){
       ...
       } else if(PMT::IsCD(copyno) and PMT::Is3inch(copyno))
           {
    ...
    while (m_start_idx<n_hit or n_hit == 0 )  {
        ...
        if (m_start_idx==0 )
          {     
                ...
                if( m_CDLPMTtotalhits >= 0 ){
                   sim_event->setLpmtHits(m_CDLPMTtotalhits);
                }
                if ( m_CDSPMTtotalhits >= 0 ){
                    sim_event->setSpmtHits(m_CDSPMTtotalhits);
                }

                if ( m_CDNNVTtotalhits >= 0){
                    sim_event->setNNVTHits(m_CDNNVTtotalhits);
                }

                if ( m_CDHamamatsutotalhits >= 0){
                    sim_event->setHamaHits(m_CDHamamatsutotalhits);
                }
...
void 
DataModelWriterWithSplit::fill_hits(JM::SimEvt* dst, const G4Event* evt)
{
    ...
    junoHit_PMT_muon_Collection* col_muon = (junoHit_PMT_muon_Collection*)(HCE->GetHC(CollID));
    // TT hits
    CollID = SDman->GetCollectionID("hitCollectionTT");
    junoHit_TT_Collection* col_tt = (junoHit_TT_Collection*)(HCE->GetHC(CollID));
    ...
    // fill Top-Tracker hits
    if (col_tt) {
      fill_hits_tt(col_tt, dst);
    }
    ...
    for (G4int index=0; index < nVertex; ++index) {
        ...
        while (pp) {
            ...
            if(!jm_trk){
               ...
               trk->setParentID(0); // use 0 to indicate it is primary track
               trk->setAncestorID(0);
               simtracksvc->put(trk);
               jm_trk=trk;
            }
...
void
DataModelWriterWithSplit::fill_tracks(JM::SimEvt* dst)
{
    if (!simtracksvc) {
    ...
    for (auto track: alltracks) {
        ...
        jm_trk->setAncestorID(track->getAncestorID());
    ...
    // vertex
    auto& allvertices = simtracksvc->allVertices();
    for (auto vertex: allvertices) {
        auto jm_vertex = dst->addVertex();
        jm_vertex->setObjID(vertex->getObjID());
        jm_vertex->setXYZT(vertex->getX(), vertex->getY(), vertex->getZ(), vertex->getT());
        jm_vertex->setProcName(vertex->getProcName());
        jm_vertex->setTrackIdxIn(vertex->getTrackIdxIn());
        jm_vertex->setTrackIdxOut(vertex->getTrackIdxOut());
    }
