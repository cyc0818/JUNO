    ...
    , m_radial(0), m_z(0)
{
    ...
    declProp("z", m_z);
    declProp("material", m_material_name="LatticedShellSteel");
}
...
HBeamConstruction::makeIt() {
    ...
    // Material
    Steel = G4Material::GetMaterial(m_material_name);
    if (!Steel) {
        LogError << "Failed to find material " << m_material_name << std::endl;
        return;
    }
