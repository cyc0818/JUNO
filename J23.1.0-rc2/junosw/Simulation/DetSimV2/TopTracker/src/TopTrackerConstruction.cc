...
TopTrackerConstruction::initVariables() {
  ...
  std::string wall_placement_file  = std::getenv("JUNO_GEOMETRY_PATH");
  wall_placement_file += "/TopTracker/TopTrackerWallPlacement.txt";

  // Keep old PATH search for compatibility, for now. Should be removed later.
  if(! boost::filesystem::exists(wall_placement_file) ){
      LogError << "Could not find file '" << wall_placement_file << "' required for constructing TT." << std::endl;
  }
...
void
TopTrackerConstruction::makeStripG4Sensitive() {

  G4SDManager* SDman = G4SDManager::GetSDMpointer();
  G4String SDname;

  auto TopTrackerSD = new junoSD_TT(SDname="TT_OPERA");
  SDman->AddNewDetector(TopTrackerSD);

  if (!logicBar) {
    makeStripLogical();
  }
  logicBar->SetSensitiveDetector(TopTrackerSD);
