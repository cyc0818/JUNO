...
#--------------------------------------------------------------------------------
  def genGetSetAttMethod(self,att,what,scopeName=''):
    orig_what = what
    desc = {'get':'Retrieve ', 'get_c':'Retrieve const ', 'set':'Update ', 'set_s': 'Only set one value', 'get_s': 'Only get one value'}
    s = ''
    ...
    if what in ['get', 'get_c'] and att['getMethScalar'] == 'TRUE':
      metName += "Vec"
    if what in ['set', 'set_s'] : metName = att['name'][0].upper() + att['name'][1:]
    ret = ''
    param = ''
    constF = ''
    if what == 'get_c' : 
    ...
    if what == 'set'  : 
      ...
      ret = 'void '
      basic_type = att['type']
      if basic_type.startswith("std::vector<") and basic_type.endswith(">"):
        basic_type = basic_type[12:-1]
      
      param = self.tools.genParamFromStrg(basic_type) + ' value'
      what = 'set'
    elif what == 'get_s'  : 
      basic_type = att['type']
      if basic_type.startswith("std::vector<") and basic_type.endswith(">"):
      ...
      ret = self.tools.genReturnFromStrg(ret + basic_type, self.generatedTypes, scopeName)
      ret += ' '
      what = ''
    ...
    if ( not scopeName ) : # this is a declaration
      ...
      elif not what and orig_what in ['get_s']:
        s += ' \n{\n  return m_%s.at(0);\n}\n\n' % att['name']
      elif not what        : s += ' \n{\n  return m_%s;\n}\n\n' % att['name']        # what was set to '' if getter
      elif orig_what == 'set_s' and what == 'set':
        s += " \n{\n  if (!m_%(name)s.size()) { m_%(name)s.push_back(value); }\n  else if (m_%(name)s.size()==1) { m_%(name)s[0] = value; } \n}\n\n" % {"name": att['name']}
...
#--------------------------------------------------------------------------------
  def genSmartRefFunctionalMethod(self, srs, scopeName=''):
    ...
    if not scopeName:
      ...
      for sr in srs:
        s += '  //Check if ' + sr['attrs']['name'] +' exists\n'
        uEventName = sr['attrs']['name'][0].upper() + sr['attrs']['name'][1:]
        s += '  bool has' + uEventName + '();\n\n'
    else:
      #definition
      ...
      for sr in srs:
        uEventName = sr['attrs']['name'][0].upper() + sr['attrs']['name'][1:]
        s += 'inline bool ' + scopeName + '::has' + uEventName + '()\n{\n'
  ...
  def genGetSetMethods(self,godClass,clname=''):
    ...
    if godClass.has_key('attribute'):
	  ...
	  if attAtt['nonconstaccessor'] == 'TRUE' :
        ...
        if attAtt['setMethScalar'] == 'TRUE':
          s += self.genGetSetAttMethod(attAtt,'set_s',clname) # this is for vector only
        if attAtt['getMethScalar'] == 'TRUE':
          s += self.genGetSetAttMethod(attAtt,'get_s',clname) # this is for vector only
