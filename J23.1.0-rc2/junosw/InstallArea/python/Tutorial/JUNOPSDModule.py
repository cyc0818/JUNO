...
class JUNOPSDModule(JUNOModule):
    ...
    def __init__(self):
        self.shared_buffer_flag = False
        pass

    def set_shared_buffer_flag(self,flag):
        self.shared_buffer_flag = flag
    ...
    def init(self, toptask, args):
        ...
        if self.shared_buffer_flag:
            self.shared_buffer(toptask)
    def shared_buffer(self,toptask):
        '''
               junotoptask
                    |
                    V
                 psdtoptask
        '''
        # shared buffer with junotop task
        junotop_task = toptask.getParent()
        md = toptask.createSvc("MappedDataSvc")
        md.regist(junotop_task, "/Event", "/Event")
