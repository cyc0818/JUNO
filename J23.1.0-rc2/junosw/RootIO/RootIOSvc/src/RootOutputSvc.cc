    ...
    : SvcBase(name), m_streamInitialized(false)
{
    ...
    declProp("IsReproducible", m_isReproducible=false);
    m_fileMgr = new RootOutputFileManager();
}
...
bool RootOutputSvc::initialize()
{
    ...
    LogInfo << "... isReproducible: "
            << m_isReproducible
            << std::endl;
    RootOutputFileHandle::setReproducible(m_isReproducible);
...
bool RootOutputSvc::initializeOutputStream(JM::EvtNavigator* nav)
{
    ...
    for (pit = priority2paths.begin(); pit != pend; ++pit) {
        ...
        m_fileMgr->new_file(m_outputFileMap[primary_path], path2priority);
        // Create output stream
        RootOutputStream* stream = new RootOutputStream(m_path2typeMap[primary_path], primary_path, m_fileMgr);
...
bool RootOutputSvc::reviseOutputStream(const std::string& path, const std::string& headerName)
{
    ...
    for (it = m_outputStreams.begin(); it != end; ++it) {
        if (path == (*it)->path()) {
            ...
            m_fileMgr->reviseOutputFile(m_outputFileMap[path], priority);
...
bool RootOutputSvc::doAttachObj(const std::string& path, TObject* obj)
{
    ...
    if (pos != m_outputFileMap.end()) {
        RootOutputFileHandle* file = m_fileMgr->get_file_with_name(pos->second);
...
bool RootOutputSvc::newFile(const map<string,string>& outputMap)
{
    // Start new files. The user needs to specify new file names for all output paths. 

    bool okay = true;
    for (auto it = m_outputFileMap.begin(); it != m_outputFileMap.end(); ++ it) {
        if (outputMap.find(it->first) == outputMap.end()) {
            okay = false;
            break;
        }
        else if (it->second == outputMap.at(it->first)) {
            okay = false;
            break;
        } 
    }
    if (!okay) {
        LogFatal << "Failed to start new file(s). "
                 << "Please check your output map which should be consistent with the initial one." 
                 << endl;
        return false;
    }

    m_outputFileMap = outputMap;
    m_path2typeMap.clear();
    m_exOutputObjs.clear();
    m_notYetInitPaths.clear();
    m_streamInitialized = false;

    okay = this->finalize() && this->initialize();
    if (!okay) {
        LogFatal << "Failed to start new file." << endl;
        return false;
    }
    m_fileMgr->reset();
    return true;   
...
void RootOutputSvc::setOutputStreams(const map<string,string>& outputStream) {
    // Do not accept changes after initialized
    if (!m_streamInitialized) m_outputFileMap = outputStream;
}
