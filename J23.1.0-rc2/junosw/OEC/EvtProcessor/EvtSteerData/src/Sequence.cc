...
std::vector<AlgBase*>& Sequence::algoVector() {
...
void Sequence::makeVectors(AlgBase* alg){
    m_sequenceAlgoVector.push_back(alg);
    m_sequenceAlgoNum++;
}
...
void Sequence::print(){
    ...
    //}
    for (auto alg : m_sequenceAlgoVector){
        LogInfo <<" Algo(s): "<< alg->objName() <<std::endl;
    }
