...
{   
    ...
    OECTagSvc *tmpTagSvc=tagsvc.data();
    //get map and cout
    m_tagMap=tmpTagSvc->getTagMap();
    //LogInfo<<"==========================tag map:"<<std::endl;
    //for (std::map<std::string, int>::const_iterator it=m_tagMap.begin();it!=m_tagMap.end();++it){
    //    LogInfo<<it->first<<"  =====>  "<<it->second<<std::endl;
    //}
    //LogInfo<<"=========================="<<std::endl;


    LogInfo << " initialized start" << std::endl;
    ...
    LogInfo<<"Seq File Path: "<<m_seqListFile<<std::endl; 
    ...
    if (!buildSignatureList(m_Signature_DocNode, true)) return false;
...
{
    ++m_count;
    LogInfo << "execute: count: " << m_count << std::endl;
    return true;
}
*/
    bool
...
#define XMLTF XMLString::transcode
    ...
    for( XMLSize_t xx = 0; xx < nodeCount; ++xx )
    {
        ...
        {
            LogInfo<<XMLString::transcode(currentNode->getNodeName())<<std::endl; 
            //解析SEQUENCE中属性值
...
bool EvtConfigSvc::buildSignatureList(const DOMDocument* xmlDoc, bool first)
{
    if ( first ) {
        first=false;
            ...
            if(XMLCF(XMLTF(currentNode->getNodeName()),"SIGNATURE")==0) {
                ...
                for(int i = 0; i < attributesCount; i++) {
                    if(XMLCF(XMLTF(attributes->item(i)->getNodeName()),"signature_id")==0){
                        ...
                        LogInfo<<"signature_id: "<<helpString<<std::endl;   
                    }
                    ...
                    if (XMLCF(XMLTF(info_currentNode->getNodeName()),"CRITERIA") == 0) {
                        ...
                        for (int i = 0; i < attributesCount; i++) {
                            if(XMLCF(XMLTF(attributes->item(i)->getNodeName()),"behaviour")==0) {
                                ...
                                LogInfo<<"behaviour: "<<behaviour<<std::endl;
                            }
                            if(XMLCF(XMLTF(attributes->item(i)->getNodeName()),"next")==0) {
                                if (behaviour != "jump") {
                                    LogInfo << "Unconsistent of behaviour and next" << std::endl;
                                ...
                                for (unsigned int i = 0; i < m_SequenceVec.size(); i++) {
                                    ...
                                    if (m_SequenceVec[i]->getSeqID() == current_sigID) {
                                        ...
                                        if (i == m_SequenceVec.size()) {
                                            LogInfo<<"i: "<<i<<" size: "<<m_SequenceVec.size()<<std::endl;
                            ...
                            if(XMLCF(XMLTF(attributes->item(i)->getNodeName()),"mark")==0) {
                                ...
                                if ((behaviour!= "accept")&&(behaviour!= "continue")) {
                                    LogInfo << "Unconsistent of behaviour and mark" << std::endl;

                                    exit(1);
                                }
                                theMark=XMLTF(attributes->item(i)->getNodeValue());
                                std::map<std::string, std::string>::iterator tagMapIt=m_tagMap.find(theMark);
                                if (tagMapIt==m_tagMap.end()){
                                    LogError<<"illegal mark!please check the sig.xml file!"<<std::endl;
                                    return false;
                                }
                                behaviour = behaviour +'_'+ theMark+"-"+tagMapIt->second;
                            }
                            //if(XMLCF(XMLTF(attributes->item(i)->getNodeName()),"tag")==0) {
                            //    if (!hasMark) {
                            //        LogInfo << "Unconsistent of mark and tag" << std::endl;

                            //        exit(1);
                            //    }
                            //    behaviour = behaviour +'-'+ XMLTF(attributes->item(i)->getNodeValue());
                            //}
                        }
                        if (behaviour.find('_') == std::string::npos) {
                            LogInfo << "Unvalid behaviour ==> " << behaviour << std::endl;

                            exit(1);
                        }
                        //EvtCriteria input Vector
                        m_EvtCriteria->setBehaviour(behaviour);
                        m_EvtCriteria->setScope(dynamic_cast<Task*>(getParent())); 
                        //每个sig下的criteria
                            ...
                            if (XMLCF(XMLTF(info_info_currentNode->getNodeName()),"ITEM") == 0) {
                                ...
                                for (int i = 0; i < 3; i++) {
                                    if(XMLCF(XMLTF(itemAttr->item(i)->getNodeName()), "name") == 0){
                                        ...
                                        LogInfo << "item name: "<<itemName << std::endl;
                                    }
    ...
    for (unsigned int i = 0; i < m_SequenceVec.size(); i++) {
        ...
        for (unsigned int j = 0; j < m_SignatureVec.size(); j++) {
            if (m_SignatureVec[j]->label() == m_SequenceVec[i]->getSeqID()) {
                ...
                LogInfo<<"Pair(Sig: "<< m_SignatureVec[j]->label()<<"--Seq: "<<m_SequenceVec[i]->getSeqID()<<")"<<std::endl;
                matched = true;
                break;
            }
        }
        if (!matched) {
            LogInfo<< "No matched Signature to Sequence(" << seqId << ")" << std::endl;
            exit(1);
        }
    }
}
