...
struct OneJob {
    ...
    struct TimeResult {
        ...
        void cal_utilization() {
            if (DEBUG) std::cout << "UTILIZATION: " << user << " " << sys << " " << real << std::endl;
        ...
        double parse_XmYs(const std::string& s) {
            ...
            while (ss.Tokenize(tok, from, "[ms]")) {
                ...
                if (DEBUG) std::cout << " LTDEBUG: tok: " << tok
                                 << " v: " << v
                                 << " from: " << from
                                 << " ss[from]: " << ss[from]
                                 << std::endl;
            }
            if (DEBUG) std::cout << " LTDEBUG orig str: " << ss << " to " << t << " s" << std::endl;
            return t;
        }
    ...
    void load_log_mem(TString fn) {
        ...
        if (DEBUG) std::cout << "dir: " << dir << " file: " << bas << std::endl;
        ...
        while(ifs.good()) {
            ...
            if (tmp_line.find("Running @") != string::npos) {
                ...
                time_result.parse(tmp_line);
                    ...
                    || tmp_line.find("[Var]EvtMax")!=string::npos) {
                parse_evtmax_sniperv1(tmp_line);
            } else if (tmp_line.find("EvtMax")!=string::npos) { // output of SNiPER v2 is JSON
                parse_evtmax_sniperv2(tmp_line);
    ...
    void parse_evtmax_sniperv1(const std::string& s) {
        ...
        while (ss.Tokenize(tok, from, "=")) {
            if (DEBUG) { std::cout << "LTDEBUG " << tok << std::endl; }
            if (tok.IsDigit()) {
                _evtmax = tok.Atoi();
            }
        }
    ...
    void parse_evtmax_sniperv2(const std::string& s) {
        // note, if evtmax is -1, we need to load root file and count the entries.
        // Because evtmax is an attribute in task, if multiple tasks are used,
        // we will find multiple evtmax.
        int _evtmax = -1; 
        ...
        TString tok;
        Ssiz_t from = 0;
        while (ss.Tokenize(tok, from, "[:,]")) { // need to use Regular Expression
            if (DEBUG) { std::cout << "LTDEBUG " << tok << std::endl; }
            if (tok.IsDigit()) {
                _evtmax = tok.Atoi();
            }
...
struct Drawer {
    ...
    void collect_jobs(const TString& inputdir) {
        ...
        std::cout << "Looking log file in " << directory << std::endl;
