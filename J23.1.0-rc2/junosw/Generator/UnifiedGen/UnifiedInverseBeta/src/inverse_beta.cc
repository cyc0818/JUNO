...
Double_t inverse_beta_prob(Double_t* x, Double_t* par)
{
  ...
  //do NOT need to multiply by the jacobian sin(theta) here!
    prob = gReactorFlux->Flux(engInMeV)
      *gInverseBeta->DSigmaDcos(engInMeV, cosThEL);
...
Double_t inverse_beta_flatProb(Double_t* x, Double_t* par)
{
  ...
  if (0<=engInMeV and engInMeV <= 12.) {
    engInMeV = 4.0; // 4MeV
    prob = 2.23341e+19*gInverseBeta->DSigmaDcos(engInMeV, cosThEL);  
...
NewRun:
  ...
  Double_t Ee = gInverseBeta->GetE_e(aEngNu, aCosThEl);
...
struct Params {
  ...
  Params() {
    ...
    fFraction235U = -1.e13;
    fFraction238U = -1.e13;
    fFraction239Pu = -1.e13;
    fFraction241Pu = -1.e13;
    fMethod = 10;
    outFilename = NULL;
  ...
  double Dm2_21;
  double absDm2_31;
  double fFraction235U;
  double fFraction238U;
  double fFraction239Pu;
  double fFraction241Pu;
  int  fMethod ;
...
int main(int argc, char** argv) {
  ...
  gReactorFlux = new ReactorFlux(); // defaults to common input
  gNuOscillations = new NuOscillations(params.normal_hierarchy);
  gInverseBeta = new IBDXSec();
  ...
  if(params.fMethod != 10) gReactorFlux->SetMethod(params.fMethod);
  if(params.fFraction235U != -1.e13 && params.fFraction238U != -1.e13 &&params.fFraction239Pu != -1.e13&& params.fFraction241Pu != -1.e13) gReactorFlux->SetFissionFraction(params.fFraction235U, params.fFraction238U, params.fFraction239Pu, params.fFraction241Pu);
  ...
  for( i=0 ; i<params.nEvents ; i++ ) {
    ...
    GenerateInverseBetaPrimaries(funcInvBetaProb, params.nu_angle_wrt_y_in_deg, 
				 ...
				 pPositron, pNeutron);
    // cout<<"ok"<<endl;
    TF1 *ffflux = new TF1("engspec",neutrino_spectrum,0.1, 10, 0);
    Double_t enu = ffflux ->GetRandom();
    ...
    if(params.debug) {
      hPositronEng->Fill(positronEng);
      hNeutrinoEng->Fill(neutrinoEng);
    }
...
void ProcessArgs(int argc, char** argv, Params& params) 
{
  ...
  for( i=1 ; i<argc ; i++ ) {
    if( strcmp(argv[i], "-seed")==0 ) {
      ...
      params.normal_hierarchy = true;
      params.oscillations = true;
    } else if( strcmp(argv[i], "-baselineinmeter")==0) {
      i++;
      sscanf(argv[i], "%lf", &params.base_line_m);
      params.oscillations = true;
    } else if ( strcmp(argv[i], "-SinSqTheta12")==0) {
      i++;
      sscanf(argv[i], "%lf", &params.sin2_th12);
      params.oscillations = true;
    } else if ( strcmp(argv[i], "-SinSqTheta13")==0) {
      i++;
      sscanf(argv[i], "%lf", &params.sin2_th13);
      params.oscillations = true;
    } else if ( strcmp(argv[i], "-DeltaM21Sq")==0) {
      i++;
      sscanf(argv[i], "%lf", &params.Dm2_21);
      params.oscillations = true;
    } else if ( strcmp(argv[i], "-AbsDeltaM31Sq")==0) {
      i++;
      ...
      params.flat = 1;
    } else if (strcmp(argv[i],"-fissionfraction")==0){
      i++;
      sscanf(argv[i], "%lf", &params.fFraction235U);
      i++;
      sscanf(argv[i], "%lf", &params.fFraction238U);
      i++;
      sscanf(argv[i], "%lf", &params.fFraction239Pu);
      i++;
      sscanf(argv[i], "%lf", &params.fFraction241Pu);
    } else if (strcmp(argv[i],"-fluxmodel")==0){
      i++;
      sscanf(argv[i], "%d", &params.fMethod);
...
void Usage() {
  printf("InverseBeta.exe [-seed seed] [-o outputfilename] [-n nevents] [-angle neutrino_angle_in_deg] [-eplus_only] [-neutron_only] [-osc] [-NH] [-IH] [-baselineinmeter] [-SinSqTheta12] [-SinSqTheta13] [-DelatM21Sq] [-AbsDeltaM31Sq] [-fissionfraction f235 f238 f239 f241] [-fluxmodel][-debug] [-flat]\n");
}
