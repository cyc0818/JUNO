...
Double_t KRLInverseBeta::Ee1(Double_t aEnu, Double_t aCosTheta) {
  Double_t answer;
  fE0 = Ee0(aEnu);
...
Double_t KRLInverseBeta::DSigDCosTh(Double_t aEnu, Double_t aCosTheta) {
...
Double_t KRLInverseBeta::GammaTerm(Double_t aCosTheta) {
...
Double_t KRLInverseBeta::SigmaTot(Double_t aEnu) {
  ...
  if( aEnu>10 ) {
    ...
    answer = fDifferential->IntegralError(-1.0, 1.0, &aEnu, 0, 1e-9);
    return answer;
  }
...
void KRLInverseBeta::setupTotalCrossSection() {
  ...
  for( int i=1 ; i<=1000 ; i++ ) {
    ...
    fHTotalCrossSection->SetBinContent(i, fDifferential->IntegralError(-1.0, 1.0, &enu, 0, 1e-9) );
...
/// inverts eqn 11
Double_t KRLInverseBeta::PromptEnergyToNeutrinoEnergy(Double_t aE_prompt) {
...
Double_t gKRLDSigmaByDCosTheta(Double_t* x, Double_t*a) {
  ...
  Double_t enu = a[0];
  if( gInverseBeta==NULL ) gInverseBeta = new KRLInverseBeta();
...
// a redirect to KRLInverseBeta::SigmaTot()
