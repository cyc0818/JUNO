   ...
   , m_ptable(NULL)
{
  ...
  declProp("PECut", PMTThreshold_pe = 2);
...
{
  LogDebug  << "configure the reconstruct tool [SpmtMuonRecTool]!"
    << std::endl; 

  m_ptable = ptab; 
  h_all1.reset(new TH2F("h_all1","",100,-180,180,100,-90,90)); //all pmt pe that > pe threshold
  // h_all1->SetDirectory(0);
  spec2.reset(new TSpectrum2(maxpositions));
  // spec2->SetDirectory(0);

  return true; 
}

  bool
SpmtMuonRecTool::reconstruct(RecTrks* trks)
{
