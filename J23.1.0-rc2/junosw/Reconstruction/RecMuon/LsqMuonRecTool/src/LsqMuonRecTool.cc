   ...
   ,m_oriptable(NULL)
{
  ...
  declProp("RelativeTimeCut",m_relativetimecut=250);
  declProp("ChargeCut",m_qcut=100);
  declProp("NPmtRequired",m_pmtUsedNumCut = 15000);
...
LsqMuonRecTool::configure(const Params* pars, const PmtTable* ptab){
  LogDebug<< " config reconstructing tool [LsqMuonRecTool] "
    <<std::endl;
...
bool 
LsqMuonRecTool::reconstruct(RecTrks* trks){
...
void 
LsqMuonRecTool::corrpmtinfo(TVector3& ri,TVector3& dir){
  if(m_corrmethod == "nocorr"){
    return;
  }
  ...
  void 
LsqMuonRecTool::iniargs(TVector3 & ri, double & ti,
    TVector3 & direction, double& pesum)// output
{
  ...
  Rchgcen = 1.5 * Rchgcen * (1/chargetotal);
...
int 
LsqMuonRecTool::pmtCut(){
  PmtTable& ptab = m_ptable;
  unsigned int pmtNum = ptab.size();
  double earliest_fht  = ptab[0].fht;
  fhts.reset(new TH1F("", "", 1500, -250, 1250));
  fhts->SetDirectory(0);
  for (unsigned int ipmt = 0; ipmt < pmtNum; ++ ipmt) {
    fhts->Fill(ptab[ipmt].fht);
  }
  earliest_fht = fhts->GetMaximumBin();
  while (0 <= earliest_fht && fhts->GetBinContent(earliest_fht) > 2) {
    earliest_fht --;
  }
  earliest_fht --;
  for (unsigned int ipmt = 0; ipmt < pmtNum; ++ipmt) {
  ...
  int pmtNumUsed = 0;
  for (unsigned int ipmt = 0; ipmt < pmtNum; ++ipmt) {
    if(ptab[ipmt].fht-earliest_fht>m_relativetimecut)
      ptab[ipmt].used = false;
    if(ptab[ipmt].used)
      pmtNumUsed ++;
  }
