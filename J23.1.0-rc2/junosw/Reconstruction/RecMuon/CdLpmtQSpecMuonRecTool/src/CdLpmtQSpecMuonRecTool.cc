...
#include "CdLpmtQSpecMuonRecTool.h"

DECLARE_TOOL(CdLpmtQSpecMuonRecTool); 

  CdLpmtQSpecMuonRecTool::CdLpmtQSpecMuonRecTool(const std::string& name)
  :ToolBase(name)
   , m_ptable(NULL)
{
  ...
  vpe_around.reserve(17612);
  PmtPos.reserve(17612);
  OptPmtPe.reserve(17612);
  vfitp.reserve(10);
  rectracks.reserve(10);
  qwpoints.reserve(10);
  reqwpoints.reserve(10);
  vrec_in.reserve(10);
  vrec_out.reserve(10);
  //declProp("FhtCorrFile", m_fname="");
}

CdLpmtQSpecMuonRecTool::~CdLpmtQSpecMuonRecTool()
{
}


  bool
CdLpmtQSpecMuonRecTool::configure(const Params* pars, const PmtTable* ptab)
{

  LogDebug  << "configure the reconstruct tool [CdLpmtQSpecMuonRecTool]!"
    << std::endl; 
  h_all1.reset(new TH2F("h_all1","",100,-180,180,100,-90,90)); //all pmt pe that > pe threshold
  spec2.reset(new TSpectrum2(maxpositions));
  ...
  m_ptable = ptab; 
    ...
    m_timerevent=m_junotimersvc->get("CDLPMTMURECEVT/timerevt");
    ...
    m_cdlpmtmurec = svc->bookTree(*m_par, "USER_OUTPUT/cdlpmtmurec", "simple output of cdlpmtmurec rec data");    
    m_cdlpmtmurec->Branch("time_total",&time_total,"time_total/F");
    N_mem = 2;   
    m_cdlpmtmurec->Branch("N_mem", &N_mem, "N_mem/I");   
    m_cdlpmtmurec->Branch("mem_resident",mem_resident,"mem_resident[N_mem]/F"); 
#endif    
  ...
  bool
CdLpmtQSpecMuonRecTool::reconstruct(RecTrks* trks)
{
  ...
  LogDebug << "Reconstructing single+multi muon tracks by CD 20inch pmt charge only ..." << std::endl;
  const PmtTable& ptab = *m_ptable;
  unsigned int pmtnum  = ptab.size();
  const double DEG = 180./TMath::Pi();
  //int maxpositions  = 7;//fitting 
  //double PMTThreshold_pe = 1000; //for fitting used, pe selected cut for each pmt, >PMTThreshold_pe cut, not >=
  //int PMTThreshold_n = 1; //for fitting used, total npmt cut, >=PMTThreshold_n, not >PMTThreshold_n
  //double AroundPePlus_DisCut = 5*m;//meter, <=AroundPePlus_DisCut, not <AroundPePlus_DisCut
  ...
  for (unsigned int i = 0; i < pmtnum; ++i)
  {
    if(ptab[i].used == 1){
      if(ptab[i].type  == _PMTINCH20){
        ...
        if(pmtq > PMTThreshold_pe){ //After elecsim, delete pmtfht<500
        ...
        // LogInfo << "- "
        //         << " copyno: " << i
        //         << " q: " << pmtq << " (PMTThreshold_pe: " << PMTThreshold_pe << ")"
        //         << " fht: " << pmtfht << " (pmtfht threshold: " << 500 << ")"
        //         << std::endl;
      }
    }
  }
  ...
  int nAllxy = spec2->Search(h_all1.get(),2,"noMarkov goff");
  ...
  if(nAllxy>0){
    ...
    int npmt_overPEcut = int(OptPmtPe.size());
    double quality = npmt_overPEcut*1.; //Borrow quality as Npmt_fired which > Qe cut
    for(unsigned int i=0; i<vrec_in.size(); i++){
      ...
      trks->addTrk(start, end, -1, -1, totalpe, quality, nFitp);
    ...
    LogInfo << "Time consumes: " << time_total << " s"<<std::endl;  
    m_cdlpmtmurec->Fill();
#endif  
...
double CdLpmtQSpecMuonRecTool::hitSphereAll(double tarR, TVector3 &center, TVector3 &vpos, TVector3 &dir, TVector3 &X1, TVector3 &X2){
  double R = tarR;
  double trackL = 0.;
