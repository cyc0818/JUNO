...
// tree_time_falling_edge is for the falling edge t~[0,1000] and h_time_rising_edge is for the rising edge t~[-200,0]
{
  ...
  if (debug_plot_fit_result) {
    ...
    TCanvas* c2 = new TCanvas(name_c2 + "_" + m_evtID);
    gPad->SetLogy();
  ...
  if (debug_plot_fit_result) {
    ...
    TCanvas* c1 = new TCanvas(name_c1 + "_" + m_evtID);
    // gPad->SetLogy();
...
bool PSD_TMVA::preProcess(JM::EvtNavigator* nav)
{
  ...
  if (debug_plot_fit_result) {
    ...
    h_time->Draw("hist");
    c3->SetLogy();
    c1->SaveAs((TString)c1->GetTitle() + "_" + m_evtID + ".png");
    c2->SaveAs((TString)c2->GetTitle() + "_" + m_evtID + ".png");
    c3->SaveAs((TString)c3->GetTitle() + "_" + m_evtID + ".png");
    delete c1;
    delete c2;
    delete c3;
  }
...
double PSD_TMVA::CalPSDVal(bool only_fill_TTree)
{
